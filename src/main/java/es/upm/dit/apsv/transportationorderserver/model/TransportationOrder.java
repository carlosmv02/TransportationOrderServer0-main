...
public class TransportationOrder {
    ...
    /** Distance from last known position to the destination (Euclidean). */
    public double distanceToDestination() {
        return Math.sqrt(
            Math.pow(this.dstLat - this.lastLat, 2) +
            Math.pow(this.dstLong - this.lastLong, 2)
        );
    }

    /** TransportationOrders are equal if they have the same truck ID. */
    @Override
    public boolean equals(Object o) {
        if (this == o) return true;
        if (!(o instanceof TransportationOrder)) return false;
        TransportationOrder other = (TransportationOrder) o;
        return truck != null && truck.equals(other.truck);
    }

    @Override
    public int hashCode() {
        return truck == null ? 0 : truck.hashCode();
    }
}
